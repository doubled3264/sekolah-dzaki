...
export default {
   ...
   actions: {
      ...
      async editInfo({}, data) {
         ...
         formData.append('imageIsEdited', data.imageIsEdited)

         if (data.imageIsEdited) {
            formData.append('thumbnail', data.raw.name)
            formData.append('itemToDelete', data.itemToDelete)
            formData.append('thumbnailImage', data.raw)
         }
      ...
      async reorderingImageItem({}, data) {
         return new Promise((resolve, reject) => {
            axios
               .post('article/image-item/reordering', data)
               .then((response) => {
                  resolve(response)
               })
               .catch((error) => {
