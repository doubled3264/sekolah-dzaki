...
<template>
  <div class="custom-datepicker">
    ...
    <DatePicker v-model="inputValue" :model-config="modelConfig" locale="id"
      :masks="{ weekdays: 'WWW', input: 'D MMMM YYYY' }">
      <template v-slot="{ inputValue, togglePopover }">
        <div class="custom-datepicker__wrapper">
          <div class="custom-datepicker__toggle-button" @click="togglePopover()">
            <CustomIcon width="24" height="24" color="white" strokeWidth="0.1" :svgIcon="calendar" />
          </div>
          <input disabled :placeholder="placeholder" :value="inputValue" />
        </div>
      </template>
    </DatePicker>
  </div>
