...
import {
  iuranScheme,
  /* iuranExistScheme, */
} from '../../utils/validation/iuran.schema'
...
const optionalFormKey = {
  ...
  baru: ['keterangan', 'interval_detail', 'inklusi'],
}
let iuranKey = {
  exist: null,
  baru: null,
...
function clearForm() {
  ...
  _.forEach(iuranKey.baru, (key) => {
    if (key != 'interval' && key != 'inklusi') {
      ...
      if (key == 'interval') iuran.value.baru[key] = '0'
    }
  })
}
...
function validateForm() {
  if (iuranOption.value == '0') {
    ...
    if (validCount == iuranKey.exist.length - optionalFormKey.exist.length) {
      submitAction()
    } else {
      Swal.fire({
        icon: 'warning',
    ...
    _.forEach(iuranKey.baru, (key) => {
      if (_.findIndex(optionalFormKey.baru, (data) => data == key) === -1) {
        if (!errorState.value.baru[key].isError) {
          validCount++
        }
      }
    })
    if (validCount == iuranKey.baru.length - optionalFormKey.baru.length) {
      submitAction()
    } else {
      Swal.fire({
        icon: 'warning',
...
async function submitAction() {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      if (iuranOption.value == '0') {
        await store
          .dispatch('variantIuran/add', iuran.value.exist)
          .then(async () => {
            toggleModal(false)
            Swal.fire({
              icon: 'success',
        ...
        await store
          .dispatch('iuran/add', iuran.value.baru)
          .then(async () => {
            toggleModal(false)
            Swal.fire({
              icon: 'success',
...
watch(
  () => multiSelect.exist.value,
  (value) => {
    iuran.value.exist.id_iuran = value.id_iuran
  }
)
watch(
  () => iuran.value.baru.interval,
  (value) => {
    if (value === '2') {
      iuran.value.baru.interval_detail = multiSelect.baru.options[0]
    } else if (value !== '2') {
      iuran.value.baru.interval_detail = ''
    }
  }
)
onMounted(() => {
    ...
    <div class="wrapper">
      ...
      <div class="table-data">
        <div class="table-data__title">
          ...
          <CustomButton title="tambah data" color="primary" @button-action="toggleModal(true)" :size="getWindowSize" />
        </div>
      </div>
    </div>
    <CustomModalOverlay v-if="overlayIsActive" @click="toggleModal(false)">
      <CustomModal>
        ...
        <template v-slot:body>
          <div class="row gap-4 my-6 border-b-2 border-verdigris">
            <div class="input-field w-6/12">
              <CustomRadioButton label="gunakan data iuran? " :items="['yang sudah ada', 'tambah data baru']"
          ...
          <div v-if="iuranOption == '0'">
            <div class="row gap-4 mb-2">
              <div class="input-field w-1/2">
                ...
                <VueMultiselect v-model="multiSelect.exist.value" :options="multiSelect.exist.options"
                  track-by="id_iuran" label="nama" placeholder="pilih salah satu">
                </VueMultiselect>
              </div>
              <div class="input-field w-1/2">
                <CustomInput type="number" label="nominal *" placeholder="masukan nominal"
                  :error-state="errorState.exist.nominal" v-model:input-value="iuran.exist.nominal"
          ...
          <div v-if="iuranOption == '1'">
            <div class="row mb-2">
              <div class="input-field w-full">
                <CustomInput type="text" label="nama iuran *" placeholder="masukan nama iuran"
                  :error-state="errorState.baru.nama" v-model:input-value="iuran.baru.nama"
                  @validate-input="validateInput('nama')" />
              </div>
            </div>
            <div class="row gap-4 mb-2">
              <div class="input-field w-1/2">
                <CustomInput type="text" label="keterangan" placeholder="masukan keterangan"
                  :error-state="errorState.baru.keterangan" v-model:input-value="iuran.baru.keterangan" />
              </div>
              <div class="input-field w-1/2">
                <CustomInput type="number" label="nominal *" placeholder="masukan nominal"
                  :error-state="errorState.baru.nominal" v-model:input-value="iuran.baru.nominal"
                  @validate-input="validateInput('nominal')" />
              </div>
            </div>
            <div class="row gap-4 mb-2">
              <div class="input-field w-6/12">
                <CustomRadioButton label="Interval *" :items="['bulanan', 'tahunan', 'lain-lain']"
                  v-model:input-value="iuran.baru.interval" />
              </div>
            </div>
            <div v-if="iuran.baru.interval == '2'" class="row gap-8 mb-2">
              <div class="input-field w-6/12">
                <VueMultiselect v-model="multiSelect.baru.value" :options="multiSelect.baru.options"
