...
async function deleteCalonSiswa() {
  await store
    .dispatch('calonSiswa/delete', { id: calonSiswaSelected.value.id })
    .then((succes) => {
      Swal.fire({
        icon: 'success',
...
<template>
  ...
  <teleport :disabled="!activeModal.tableAction" to="#modal">
    <CustomModalOverlayVue v-show="activeModal.tableAction" @close-modal="toggleShowOption">
      <TableAction @delete-action="deleteAction" @info-action="infoAction"></TableAction>
