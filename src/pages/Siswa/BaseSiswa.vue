    ...
    <div class="wrapper">
      ...
      <div class="table-data">
        <div class="table-data__title">
          <h4>data siswa</h4>
          <CustomButton title="tambah data" color="verdigris" @button-action="toggleModal(true, 'add')"
            :size="getWindowSize" />
        </div>

        <vue-good-table v-if="
           !tableData.columns.length == 0 && !tableData.rows.length == 0
        " :columns="tableData.columns" :rows="tableData.rows" styleClass="vgt-table striped" v-on:row-dblclick="
  ...
  <CustomOverlay v-show="overlayIsActive" @click="toggleModal(false, 'all')">
    <CustomModal v-if="modal.add">
      ...
      <template v-slot:footer>
        ...
        <div class="mt-6">
          <CustomButton title="simpan data" variant="solid" color="verdigris" size="md" block
