...
<template>
  <div :style="{ maxHeight: availHeight + 'px' }" class="article-add__preview">
    ...
    <ul class="article-add__preview-list">
      <li v-for="(item, index) in article.ArticleDetails" :key="index + 1" class="article-add__preview-item">
        <div v-if="item.type === 'text'" class="text-content">
          <div class="text__wrapper" v-html="item.content"></div>
          <div class="text-content__nav" @mouseenter="$emit('toggleShowOptions', index)"
            ...
            <CustomThreeDotOptionsList :is-show="item.showOptions">
              ...
              <CustomThreeDotOptionsItem>
                <div class="flex gap-4" @click="$emit('removeText', index)">
        ...
        <div v-if="item.type === 'image'" class="image-content">
          <div class="image__wrapper">
            <img crossorigin="anonymous" :src="`http://localhost:3000/files/article/${article.id}/${item.content}`"
              alt="" />
          </div>
          <p>{{ item.caption }}</p>
          <div class="image-content__nav" @mouseenter="$emit('toggleShowOptions', index)"
            @mouseleave="$emit('toggleShowOptions', index)">
            <CustomThreeDotOptionsList :is-show="item.showOptions">
              <CustomThreeDotOptionsItem v-if="index != 0">
                <div class="flex gap-4" @click="$emit('reorderingImage', index, 'go up')">
                  <CustomIcon :svg-icon="arrowSolidUp" />
                  <p>atas</p>
                </div>
              </CustomThreeDotOptionsItem>
              <CustomThreeDotOptionsItem v-if="index != article.ArticleDetails.length - 1">
                <div class="flex gap-4" @click="$emit('reorderingImage', index, 'go down')">
                  <CustomIcon :svg-icon="arrowSolidDown" />
                  <p>bawah</p>
                </div>
              </CustomThreeDotOptionsItem>
              <CustomThreeDotOptionsItem>
                <div class="flex gap-4" @click="$emit('editImage', index)">
                  <CustomIcon :svg-icon="imageEdit" />
                  <p>ubah</p>
                </div>
              </CustomThreeDotOptionsItem>
              <CustomThreeDotOptionsItem>
                <div class="flex gap-4" @click="$emit('removeImage', index)">
                  <CustomIcon :svg-icon="trash" />
                  <p>hapus</p>
                </div>
              </CustomThreeDotOptionsItem>
