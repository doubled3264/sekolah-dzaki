...
import { arrowSolidDown, arrowSolidUp, imageAdd, imageEdit, trash } from '../../utils/svg-vars'
...
<template>
      ...
      <div class="content__body">
        <div class="banner__wrapper">
          <div class="card">
            ...
            <div :style="{ maxHeight: availHeight + 'px' }" class="card__body">
              <ul class="banner__image-list">
                <li v-for="(item, index) in banner" :key="index + 1" class="banner__image-item">
                  <div class="image-content">
                    <img crossorigin="anonymous"
                      :src="`http://localhost:3000/files/banner/${item.placement}/${item.banner}`" alt="" />
                  </div>
                  <div class="image-content__nav" @mouseenter="toggleShowOptions(index)"
                    @mouseleave="toggleShowOptions(index)">
                    <CustomKebabMenuList :is-show="item.showOptions">
                      <CustomKebabMenuItem v-show="index != 0" @on-click="editBanner(index)">
                        <CustomIcon :svg-icon="arrowSolidUp" />
                        <p>pindah ke atas</p>
                      </CustomKebabMenuItem>
                      <CustomKebabMenuItem v-show="index != banner.length - 1" @on-click="editBanner(index)">
                        <CustomIcon :svg-icon="arrowSolidDown" />
                        <p>pindah ke bawah</p>
                      </CustomKebabMenuItem>
                      <CustomKebabMenuItem @on-click="editBanner(index)">
                        <CustomIcon :svg-icon="imageEdit" />
                        <p>ubah</p>
                      </CustomKebabMenuItem>
                      <CustomKebabMenuItem @on-click="removeBanner(index)">
  ...
  <Teleport to="#modal">
    <CustomModalOverlay v-show="modal.imageEditor" @close-modal="closeImageModal">
      <AddNewImage :image-to-edit="imageToEdit" :purpose="imageEditorPurpose" @on-close-modal="closeImageModal"
        @on-add-image="pushNewImage" @on-edit-image="pushEditedImage" />
    </CustomModalOverlay>
  </Teleport>
