...
// watch the props instead route.path to update component
watch(props, async () => {
  await restartComponent()
})
                ...
                <li v-for="(item, index) in banner" :key="index + 1" class="banner__image-item">
                  ...
                  <div class="image-content__nav" @mouseenter="toggleShowOptions(index)"
                    ...
                    <CustomKebabMenuList :is-show="item.showOptions">
                      <CustomKebabMenuItem v-show="index != 0" @on-click="reorderingBanner(index, 'go up')">
                        <CustomIcon :svg-icon="arrowSolidUp" />
                        <p>pindah ke atas</p>
                      </CustomKebabMenuItem>
                      <CustomKebabMenuItem v-show="index != banner.length - 1" @on-click="
