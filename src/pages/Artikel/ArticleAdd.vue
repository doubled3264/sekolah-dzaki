...
<template>
        ...
        <div class="card artikel-add">
          ...
          <div class="card__body">
            ...
            <ul class="artikel-add__preview-list">
              <li v-for="(item, index) in artikel.item" class="artikel-add__preview-item">
                <div v-if="item.type === 'text'" v-html="item.value"></div>
                <div v-if="item.type === 'image'" class="image-content">
                  <img :src="item.value.previewImage" :alt="item.value.rawImage.name" />
  ...
  <teleport to="#modal">
    ...
    <CustomModalOverlay v-show="modal.imageEditor" @close-modal="toggleModal('imageEditor')">
      <AddNewImage />
    </CustomModalOverlay>
  </teleport>
</template>
